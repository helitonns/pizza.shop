...
export function OrderStatus({status} : OrderStatusProps){
  return(
    <div className="flex items-center gap-2">
      ...
      {status === "processing" && (<span data-testid="badge" className="w-2 h-2 rounded-full bg-cyan-500" />)}
      {status === "delivering" && (<span data-testid="badge" className="w-2 h-2 rounded-full bg-amber-500" />)}
